...
public class WgetUtil {
    ...
    public static boolean wgetHttp(String url, String path, int tryNnm, int timeout, String username, String password) {
        ...
        if (StringUtils.isBlank(username) || StringUtils.isBlank(password)) {
            log.error("wget http failed, because username && password is empty.");
            return false;
        }
        if (tryNnm <= 0) {
        ...
        return wget(path, assemblyCmd(url, path, tryNnm, timeout, username, password, Boolean.FALSE), tryNnm);
    }
    ...
    public static boolean wgetFtp(String url, String path, String username, String password, int tryNnm, int timeout) {
        ...
        return wget(path, assemblyCmd(url, path, tryNnm, timeout, username, password, Boolean.TRUE), tryNnm);
    }
    ...
    private static String assemblyCmd(String url, String path, int tryNnm, int timeout, String username, String password, boolean isFtp) {
        StringBuilder cmd = new StringBuilder("wget -c ").append(url).append(" -O ").append(path);
        if (tryNnm > 0) {
            cmd.append(" -t ").append(tryNnm);
        }
        if (timeout > 0) {
            cmd.append(" -T ").append(timeout).append(" -w ").append(timeout);
        }
        if (StringUtils.isNotBlank(username) && StringUtils.isNotBlank(password)) {
            if (isFtp) {
                cmd.append(" --ftp-user=").append(username).append(" --ftp-password=").append(password);
            } else {
                cmd.append(" --http-user=").append(username).append(" --http-password=").append(password);
            }
        }
        if (isFtp) {
    ...
    private static boolean wget(String path, String command, int tryNnm) {
        ...
        do {
            try {
                ...
                int i = proc.waitFor();
                if (i == 0) {
                    log.info("wget success, command: {}", command);
                    return true;
                }
            } catch (Exception e) {
                log.error("wget failed, cmd: {}.", command, e);
            }
            num++;
        } while (num < tryNnm);
        return false;
