...
public class WebappLoginController {
    ...
    private ValueOperations<String, YanwuUser> loginTokenOperations;
    ...
    public ResponseEntity<ResponseEnvelope<YanwuUser>> login(@RequestBody LoginVO vo) throws Exception {
        // ----- 根据 用户名 邮箱 手机号 检索用户
    ...
    public ResponseEntity<ResponseEnvelope<Boolean>> logout(@PathVariable("id") Long id) throws Exception {
        Boolean result;
        YanwuUser user = loginTokenOperations.get(Contents.LOGIN_TOKEN + id);
        if (user == null) {
            result = true;
        } else {
